...
# Enable-Features.ps1
# Batch-enables all ViVeTool feature IDs from FeatureCatalog.ps1.
...
$LogFile = Join-Path $LogDir "enable_$Stamp.log"
...
Write-Log "=== ViVeTool Feature Enabler ==="
...
Write-Log "Catalog: $($AllFeatureIDs.Count) unique feature IDs."
...
foreach ($id in $AllFeatureIDs) {
    ...
    Write-Progress -Activity "Enabling features" -Status "$i / $($AllFeatureIDs.Count) -- ID: $id" -PercentComplete $pct
    if ($WhatIfPreference) {
        Write-Log "[WHATIF] Would run: $ViveToolPath /enable /id:$id" "WARN"
        $Results.Add([PSCustomObject]@{ID=$id;Status="WhatIf";Output="";Timestamp=(Get-Date)})
        continue
    }
    try {
        $output   = & $ViveToolPath /enable /id:$id 2>&1
        $exitCode = $LASTEXITCODE
        $outStr   = ($output -join " ").Trim()
        if ($exitCode -eq 0) {
            Write-Log "SUCCESS  ID:$id  $outStr" "SUCCESS"
            $Results.Add([PSCustomObject]@{ID=$id;Status="Success";Output=$outStr;Timestamp=(Get-Date)})
        } elseif ($outStr -match "not found|unknown|unsupported|no feature") {
            Write-Log "SKIP     ID:$id  (Unsupported) $outStr" "SKIP"
...
Write-Progress -Activity "Enabling features" -Completed
...
Write-Log "=== SUMMARY === Total:$($Results.Count) Success:$success Unsupported:$unsupported Errors:$errors"
$csv = Join-Path $LogDir "enable_$Stamp.csv"
